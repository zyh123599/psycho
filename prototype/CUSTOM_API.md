...
本文件描述心潮 V0.7 前端对用户自定义模型服务商的要求。它不是心潮自建的业务 API；当前应用没有后端。讯飞实时语音转写使用独立 WebSocket 协议，见 [`REALTIME_ASR.md`](REALTIME_ASR.md)。
...
- `xinchao.quick-notes.v1` 只持久化闪念 ID、用户确认的文字（包括语音转写）、时间戳、日历日期和图片/语音表达类型元数据；图片 base64 与录音数据都不会写入；
...
- 心潮不持久化原图、音频文件和聊天原文；讯飞会收到录音期间的实时 PCM，模型服务商会收到用户确认发送的文字或图片，它们是否记录请求由各自政策决定；
